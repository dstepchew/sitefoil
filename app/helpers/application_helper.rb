...
<div id=sitefoil_popup_overlay style="z-index:10000; position:fixed; top:0px; left:0px; width:100%; height:100%; background:#000; opacity:0"></div><div id=sitefoil_popup style="opacity:0; display:none; padding:20px; box-shadow:rgba(0, 0, 0, 0.4) 0px 0px 10px 0px; background:#fff; top:0px;animation-top:2s; position:fixed; z-index:10001;"> Your HTML goes here <center> <input style="background: url(https://s3-us-west-2.amazonaws.com/sitefoil/images/x.png)no-repeat;width:14px;height:14px;position:absolute;right:5px;top:5px;cursor:pointer;z-index:20000;border:none;" type=button id=sitefoil_popup_ok onclick=sitefoil.popup_close() value=""></center> </div> - NOTE: Use the check box below to limit the number of times the popup displays to once per session (off by default)', js: 'sitefoil.popup_show({html: ":html", once_per_session: :once_per_session, recipe_id: recipe_id})', params: [{name: :html, input_type: :textarea },{name: :once_per_session, label: "show only once per session", input_type: :checkbox}] },
      { name: 'show hidden element', help:'This action will override the display property of any matching element so that it is display:block', js: 'sitefoil.$(":selector").show()', params: [:selector]},
    	...
    	{ name: 'autofill', help:'This action will force your custom text into any field you choose based on a designated input ID', js: 'sitefoil.autofill(":selector",":value")', params:[{name: :selector, input_type: :selector_pick},:value] },
